...
class MultiplexerClientTest extends \PHPUnit_Framework_TestCase
{
    public function testBan()
    {
        ...
        $mockClient1 = \Mockery::mock(Ban::class)
            ->shouldReceive('ban')
            ->once()
            ->with($headers)
            ->getMock();

        $mockClient2 = \Mockery::mock(Ban::class)
            ->shouldReceive('ban')
            ->once()
            ->with($headers)
            ->getMock();
    ...
    public function testRefresh()
    {
        ...
        $mockClient1 = \Mockery::mock(Refresh::class)
            ->shouldReceive('refresh')
            ->once()
            ->with($url, $headers)
            ->getMock();
        $mockClient2 = \Mockery::mock(Refresh::class)
            ->shouldReceive('refresh')
            ->once()
            ->with($url, $headers)
            ->getMock();
    ...
    public function testPurge()
    {
        ...
        $mockClient1 = \Mockery::mock(Purge::class)
            ->shouldReceive('purge')
            ->once()
            ->with($url, $headers)
            ->getMock();
        $mockClient2 = \Mockery::mock(Purge::class)
            ->shouldReceive('purge')
            ->with($url, $headers)
            ->getMock();
        $mockClient3 = \Mockery::mock(ProxyClient::class);
